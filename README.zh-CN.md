...
被销毁令牌的黑名单条目会保留到「令牌自然过期时间 **+** 完整 `refresh_ttl` 刷新窗口」之后，因此已登出的令牌不可能通过刷新"复活"。传入第二个参数 `true` 可将条目保留 1 年（即"永久"）：`$manager->invalidate($token, true)`。
...
> **并发语义说明**：宽限期保证并发*校验*旧 Token 的请求不会失败；但拉黑操作本身是对缓存的先查后写（非原子），两个恰好同一瞬间到达的刷新请求可能都成功并各自拿到新 Token（旧 Token 最终仍会被拉黑）。若业务要求严格的刷新单次使用，请在 `refreshToken()` 外层自行加分布式锁。
