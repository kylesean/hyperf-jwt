...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Not Before (nbf) 偏移量 (Not Before Offset)
    |--------------------------------------------------------------------------
    |
    | 指定 'nbf' (Not Before) 声明相对于 'iat' (Issued At) 的偏移量，单位为秒。
    | 签发的令牌始终包含 nbf 声明（nbf = iat + 偏移量）。
    | 例如，设置为 -60 表示令牌将在签发前 60 秒开始生效（用于处理服务器时钟不同步）。
    | 设置为正数表示令牌将在签发后的指定秒数开始才生效。
    | 通常保持为 0 即可。
    |
    */
    'nbf_offset_seconds' => env('JWT_NBF_OFFSET_SECONDS', 0), // 单位：秒
    ...
    | 你可以设置为你的应用名称或 URL。
    | 如果设置为 null 或空字符串，则不会在 JWT 中添加此声明，
    | 并且 required_claims 中的 iss 校验也会被跳过。
    |
    */
    ...
    | 你可以设置为你的应用名称或 URL，也支持字符串数组（多受众）。
    | 如果设置为 null 或空字符串/空数组，则不会在 JWT 中添加此声明，
    | 并且 required_claims 中的 aud 校验也会被跳过。
    |
    */
    ...
    */
    ...
    |
    | 指定在验证令牌时，哪些声明是强制要求的：
    | 'iat' (Issued At) - 签发时间，为 true 时令牌缺少 iat 会被拒绝
    | 'nbf' (Not Before) - 生效时间，为 true 时令牌缺少 nbf 会被拒绝
    | 'exp' (Expiration Time) - 过期时间，为 true 时令牌缺少 exp 会被拒绝
    |   （强烈建议保持 true，否则没有 exp 的令牌将被视为永不过期）
    | 'iss' / 'aud' - 为 true 时不仅要求存在，还必须与上面配置的 issuer / audience 一致
    |   （issuer / audience 为空时对应的校验自动跳过）
    | 也可以添加自定义声明名（值为 true），表示令牌必须包含该声明。
    | 建议全部保留为 true 以增强安全性。
    |
    */
    'required_claims' => [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | 黑名单缓存键前缀 (Blacklist Cache Key Prefix)
    |--------------------------------------------------------------------------
    |
    | 黑名单条目写入缓存时使用的键前缀（键名为 前缀 + jti 的 sha256）。
    | 当多个应用共享同一个缓存驱动时，可以用不同前缀避免互相影响。
    |
    */
    'blacklist_cache_prefix' => env('JWT_BLACKLIST_CACHE_PREFIX', 'jwt_blacklist_'),
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | 黑名单并发宽限期 (Blacklist Concurrency Grace Period)
    |--------------------------------------------------------------------------
    |
    | 在高并发（如 Swoole 协程）环境下，客户端在刷新 Token 的瞬间可能会发出多个并发请求。
    | 为了防止其他并发请求因为旧 Token 被立即拉黑而报 401 错误，可以配置一个极短的宽限期（秒）。
    | 在此宽限期内，被刷新拉黑的旧 Token 仍会被校验为有效。
    | 设置为 0 表示不启用宽限期，即刷新后立即完全失效。
    |
    */
    'blacklist_concurrency_grace_period' => env('JWT_BLACKLIST_CONCURRENCY_GRACE_PERIOD', 30),
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | 控制 JwtAuthMiddleware 的行为。
    |
    */
    'middleware' => [
        // 是否强制要求认证。
        // 如果设为 true（默认），未提供 Token 或 Token 无效时会抛出异常。
        // 如果设为 false，未提供 Token 时会继续执行后续业务逻辑。
        'auth_required' => true,
    ],
